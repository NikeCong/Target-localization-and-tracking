...
if M>0
    CenEst_Target.x=sum(X(:,1))/M;     %质心算法估计位置x
    CenEst_Target.y=sum(X(:,2))/M;     %质心算法估计位置y
    CenError_Dist=getDist(CenEst_Target,Target); %目标真实位置与质心估计位置的偏差距离
end
...
Error_Dist=getDist(Est_Target,Target);      %目标真实位置与权重质心估计位置的偏差距离
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure      %画图
